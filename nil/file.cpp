...
namespace nil
{
	...
	bool read_lines(std::string const & file_name, std::vector<std::string> & output)
	{
		...
		if(result == false)
			return false;
		//std::cout << "Erasing stuff" << std::endl;
	...
	std::string get_file_size_string(ullong file_size)
	{
		...
		std::size_t offset = 0;
		double progress = static_cast<double>(file_size);
		for(double const power = 1024.0; progress > power && offset < nil::countof(units) - 1; offset++)
			progress /= power;

		std::stringstream stream;
		stream.precision(2);
		stream << std::fixed << progress << " " << units[offset];
...
#else
		return ::mkdir(path.c_str(), O_CREAT) == 0;
#endif
	}
}
